...
-- Reconciliation migration: caregiver_records, caregiver_record_availability,
-- and caregiver_record_credentials already exist live (created directly
-- against the project outside a tracked migration, during earlier
-- applicant/hiring-pipeline work) but were never captured in this
-- migrations directory - a real repo/database drift this backfills so a
-- fresh environment matches production. Every statement below is
-- idempotent (IF NOT EXISTS / DO-block guards) precisely because it is
-- already applied live; this file's job is git history parity, not a new
-- schema change.
--
-- This IS the caregiver-profile-decoupling table the Ogevia Architecture
-- Reset's items 16-18 called for (a caregiver record distinct from a
-- login account, linked_user_id nullable until someone accepts an
-- invite) - reused here rather than creating a second, duplicate
-- "caregivers" table, per that same directive's own "don't create
-- duplicate concepts, choose one canonical model" rule. See
-- 20260814070000_wire_up_caregiver_records.sql for the RPC/edge-function
-- work that actually connects it to the Team page.
