...
-- Add "applicant" as a 7th global_search() result type, same reasoning
-- and same pattern as 20260722010000's "service" branch: job_applicants
-- (supabase/migrations/20260726190000_job_applicants.sql) didn't exist
-- yet when global_search() was first written, and applicants have since
-- grown into a full first-class workflow (Builds 002-007) with their own
-- detail page - the most obvious gap left now. Reuses applicants.read,
-- no own-row carve-out (unlike credentials/incidents), matching
-- list_applicants' own access rule. job_applicants has no deleted_at
-- column (no soft delete on this table), so unlike client/authorization/
-- incident/service there's no matching filter to add here.
