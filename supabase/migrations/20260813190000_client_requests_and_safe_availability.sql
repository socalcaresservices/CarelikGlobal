...
-- Some established installations have the later shift-coverage extension,
-- which adds two OUT columns to this RPC. PostgreSQL cannot replace a function
-- when its OUT row type differs, so preserve that extension when present.
drop function if exists public.list_shifts(uuid, timestamptz, timestamptz);
do $migration$
begin
  if to_regclass('public.shift_coverage_events') is not null then
    execute $sql$
      ...
      returns table (id uuid, client_id uuid, client_name text, caregiver_user_id uuid, caregiver_name text, starts_at timestamptz, ends_at timestamptz, status public.shift_status, notes text, needs_coverage boolean, call_out_reason text)
