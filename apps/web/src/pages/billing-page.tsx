...
export function BillingPage() {
  ...
  function invalidateAll() {
    void queryClient.invalidateQueries({ queryKey: ["billing-ready-visits", activeOrganizationId] });
  ...
  const submissionItemsQuery = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.rpc("list_billing_submission_items", {
  ...
  function exportSubmissionCsv(submission: SubmissionRow, items: SubmissionItemRow[]) {
    const header = ["Client", "Service", "Date", "Minutes", "Rate", "Amount", "Status"];
    const rows = items.map((item) => [
      ...
      item.submitted_amount_cents !== null ? (item.submitted_amount_cents / 100).toFixed(2) : "",
      item.is_voided ? `Voided: ${item.void_reason ?? ""}` : "Active"
    ]);
    downloadCsv(`billing-submission-${submission.submitted_at.slice(0, 10)}.csv`, header, rows);
  }

  const unsubmittedApprovals = (approvalsQuery.data ?? []).filter((row) => !row.is_voided && !row.is_submitted);
  ...
  return (
    <section className="mx-auto max-w-6xl space-y-6">
      <PageHeader
        eyebrow="Billing"
        title="Approve and submit visits for billing"
        description="Turn signed visits into approved, dollar-valued billing batches - payer submission reports and private-pay invoices both start here."
      />

      <Card>
        <h3 className="font-semibold text-slate-950">Ready to bill ({readyQuery.data?.length ?? 0})</h3>
        <p className="mt-1 text-xs text-slate-500">
          Signed visits with no active billing approval yet. The estimated amount uses the visit&apos;s authorization
          rate - approving lets you adjust the billed minutes before it&apos;s locked in.
        </p>
        {readyQuery.isLoading ? (
          ...
          <p className="mt-3 text-sm text-slate-400">No signed visits are waiting to be billed.</p>
        ) : (
                ...
                {(readyQuery.data ?? []).map((row) => (
                  <Fragment key={row.visit_id}>
                    <tr className="border-b border-slate-100 last:border-0">
                      <td className="py-2 pr-3 whitespace-nowrap text-slate-600">{formatVisitDate(row.service_date)}</td>
                      <td className="py-2 pr-3 text-slate-800">{row.client_name}</td>
                      <td className="py-2 pr-3 text-slate-600">{row.service_name}</td>
                      <td className="py-2 pr-3 text-slate-600">{row.caregiver_name}</td>
                      <td className="py-2 pr-3 text-slate-700">{formatHours(row.billable_minutes)}h</td>
                      <td className="py-2 pr-3 text-slate-600">
                        {row.rate_cents !== null ? `${formatMoney(row.rate_cents)}/hr` : "No rate set"}
                      </td>
                      <td className="py-2 pr-3 font-medium text-slate-900">{formatMoney(row.estimated_amount_cents)}</td>
                      {canApprove ? (
                        <td className="py-2">
                          <button
                            type="button"
                    ...
                    {approvingVisitId === row.visit_id ? (
                      <tr className="border-b border-slate-100 bg-slate-50">
                        <td colSpan={canApprove ? 8 : 7} className="p-4">
                          <p className="text-sm font-semibold text-slate-900">Approve for billing</p>
                          <div className="mt-3 grid gap-3 sm:grid-cols-3">
                            <div>
                              <label htmlFor="approve-minutes" className="block text-xs font-medium text-slate-600">
                                Approved minutes
                              </label>
                              <input
                                id="approve-minutes"
                                ...
                                onChange={(event) => setApproveMinutes(event.target.value)}
                                className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm text-slate-900"
                              />
                            </div>
                            <div className="sm:col-span-2">
                              <label htmlFor="approve-notes" className="block text-xs font-medium text-slate-600">
                                Notes (optional)
                              </label>
                              <input
                                id="approve-notes"
                          ...
                          {approveError ? <p className="mt-2 text-sm text-red-700">{approveError}</p> : null}
                          <div className="mt-3 flex gap-3">
                            <Button type="button" loading={saving} onClick={() => submitApproval(row.visit_id)}>
      ...
      <Card>
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h3 className="font-semibold text-slate-950">Approved, not yet submitted ({unsubmittedApprovals.length})</h3>
            <p className="mt-1 text-xs text-slate-500">Select approvals and submit them together as one batch.</p>
          </div>
          {canSubmit ? (
            <Button type="button" loading={submitting} disabled={selectedApprovalIds.size === 0} onClick={handleSubmitBatch}>
              {submitting ? "Submitting…" : `Submit selected (${selectedApprovalIds.size})`}
            </Button>
          ) : null}
        </div>
        ...
        {approvalsQuery.isLoading ? (
          ...
          <p className="mt-3 text-sm text-red-700">Could not load billing approvals.</p>
        ) : unsubmittedApprovals.length === 0 ? (
          <p className="mt-3 text-sm text-slate-400">No approved visits are waiting to be submitted.</p>
        ) : (
          <div className="mt-4 overflow-x-auto">
            <table className="w-full text-left text-sm">
              <thead>
              ...
              <tbody>
                {unsubmittedApprovals.map((row) => (
                  <tr key={row.approval_id} className="border-b border-slate-100 last:border-0">
                    {canSubmit ? (
                    ...
                    <td className="py-2 pr-3 text-slate-800">{row.client_name}</td>
                    <td className="py-2 pr-3 text-slate-600">{row.service_name}</td>
                    <td className="py-2 pr-3 text-slate-700">{formatHours(row.approved_minutes)}h</td>
                    <td className="py-2 pr-3 font-medium text-slate-900">{formatMoney(row.amount_cents)}</td>
                    <td className="py-2 pr-3 text-slate-600">{row.approved_by_name}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <Card>
        <h3 className="font-semibold text-slate-950">Submissions ({submissionsQuery.data?.length ?? 0})</h3>
        {submissionsQuery.isLoading ? (
          <p className="mt-3 text-sm text-slate-500">Loading…</p>
        ) : submissionsQuery.isError ? (
          <p className="mt-3 text-sm text-red-700">Could not load billing submissions.</p>
                ...
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    <p className="text-sm font-medium text-slate-900">
                      ...
                      <span className="ml-2 text-slate-400">by {submission.submitted_by_name}</span>
                    </p>
                    <p className="mt-0.5 text-xs text-slate-500">
                      {submission.active_item_count} of {submission.item_count} items ·{" "}
                      {formatHours(submission.total_submitted_minutes)}h · {formatMoney(submission.total_amount_cents)}
                    </p>
                  </div>
                  <div className="flex items-center gap-2">
                    {submission.notes ? <StatusBadge label={submission.notes} tone="info" /> : null}
                    <button
                      type="button"
                      onClick={() =>
                        setExpandedSubmissionId(
                          expandedSubmissionId === submission.submission_id ? null : submission.submission_id
                        )
                      }
                      className="text-xs font-medium text-sky-700 underline-offset-2 hover:underline"
                    >
                      {expandedSubmissionId === submission.submission_id ? "Hide" : "View items"}
                    </button>
                  </div>
                </div>
                  ...
                  <div className="mt-3 rounded-lg border border-slate-200 p-3">
                    <div className="flex items-center justify-between">
                      ...
                      <Button
                        type="button"
                        variant="secondary"
                        disabled={(submissionItemsQuery.data ?? []).length === 0}
                        onClick={() => exportSubmissionCsv(submission, submissionItemsQuery.data ?? [])}
                      >
                    ...
                    {submissionItemsQuery.isLoading ? (
                      ...
                      <ul className="mt-2 divide-y divide-slate-100">
                        {(submissionItemsQuery.data ?? []).map((item) => (
                          <li key={item.item_id} className="flex items-center justify-between py-1.5 text-sm">
                            <span className="text-slate-700">
                              {item.client_name} · {item.service_name} · {formatVisitDate(item.service_date)}
                            </span>
                            <span className="flex items-center gap-2">
                              {item.is_voided ? <StatusBadge label="Voided" tone="neutral" /> : null}
                              <span className="font-medium text-slate-900">
                                {formatHours(item.submitted_minutes)}h · {formatMoney(item.submitted_amount_cents)}
                              </span>
                            </span>
                          </li>
                        ))}
