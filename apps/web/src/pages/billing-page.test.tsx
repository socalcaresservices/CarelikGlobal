...
function baseOrganization(canApprove = true, canSubmit = true) {
  return {
    organizations: [],
    activeOrganization: {
      id: ORG_ID,
...
function mockRpc({
  ...
  mockedRpc.mockImplementation((fn: string) => {
    if (fn === "list_billing_ready_visits") return Promise.resolve({ data: ready, error: null }) as never;
    if (fn === "list_billing_approvals") return Promise.resolve({ data: approvals, error: null }) as never;
    if (fn === "list_billing_submissions") return Promise.resolve({ data: submissions, error: null }) as never;
    if (fn === "list_billing_submission_items") return Promise.resolve({ data: items, error: null }) as never;
    if (fn === "approve_visit_for_billing") return Promise.resolve({ data: null, error: null }) as never;
    if (fn === "submit_billing_approvals") return Promise.resolve({ data: "sub-1", error: null }) as never;
...
describe("BillingPage", () => {
  ...
  it("approves a ready visit for billing", async () => {
    ...
    await waitFor(() => expect(screen.getByLabelText("Approved minutes")).toBeInTheDocument());
    fireEvent.click(screen.getByRole("button", { name: "Confirm approval" }));

    await waitFor(() =>
      expect(mockedRpc).toHaveBeenCalledWith(
        "approve_visit_for_billing",
        expect.objectContaining({ target_visit_id: VISIT_ID, approved_minutes: 120 })
      )
    );
  });
  ...
  it("submits selected approvals as a batch", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Submit selected (1)" }));

    await waitFor(() =>
      expect(mockedRpc).toHaveBeenCalledWith(
        "submit_billing_approvals",
        expect.objectContaining({ target_organization_id: ORG_ID, approval_ids: [APPROVAL_ID] })
      )
    );
  });
  ...
  it("exports a submission's line items as CSV", async () => {
    ...
    renderPage();
    await waitFor(() => expect(screen.getByText(/by Sam Manager/)).toBeInTheDocument());
