...
import { ClientOperationsDashboard } from "@/components/client-operations-dashboard";
...
export function ClientsPage() {
  ...
  const [activeView, setActiveView] = useState<"operations" | "records">(
    "operations",
  );
  ...
  return (
    <section className="mx-auto max-w-7xl space-y-6">
      <div>
        <p className="text-sm font-medium text-slate-500">Client Operations</p>
        <h2 className="mt-1 text-2xl font-semibold text-slate-950">
          {activeOrganization?.displayName ?? "Client records"}
        </h2>
        <p className="mt-2 max-w-3xl text-sm text-slate-600">
          See service delivery, remaining authorized hours, staffing gaps, and
          monthly shortfall reasons. Open Client Records only when you need to
          add or edit profile information.
        </p>
      </div>
      ...
      {activeView === "operations" && activeOrganizationId ? (
