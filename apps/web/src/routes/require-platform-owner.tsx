...
// The single gate for everything under admin.ogevia.com. PlatformShell's
// chrome and every platform page (Organizations, Subscriptions, Feature
// Flags, Audit, ...) sat behind ProtectedRoute (authenticated) alone,
...
// told them "Not available" one click away. Wrapping the whole admin
...
// isPlatformOwner (user_profiles.platform_role = 'platform_owner', via
// is_platform_owner() - see
...
export function RequirePlatformOwner({ children }: PropsWithChildren) {
  const { isPlatformOwner, loading } = useOrganization();
